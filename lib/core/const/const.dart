const String loginScreen="/logic-screen";
const String registerScreen="/register-screen";
const String homeScreen="/home-screen";
const String splashScreen="/splash-screen";
const String quizScreen="/quiz-screen";
const String userData="/user-data";
const String chatScreen="/chat-screen";
const String typeRegisterScreen="/studentRegister-screen";
const String homeDeciderScreen="/homeDecider-screen";
const String paymentData="/paymentData-screen";
const String paymentSubmit="/paymentSubmit-screen";






